...
function selectDate(iso){
 selectedDate=iso;const [y,m,d]=iso.split('-').map(Number),map=dayMap(),live=liveToday(),row=map.get(iso)||(live?.date===iso?live:null);
...
async function init(){
 ...
 year=Math.max(...data.years);setMode('tmean');setYear(year);
}
init();