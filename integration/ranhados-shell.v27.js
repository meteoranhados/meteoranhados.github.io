(() => {
  ...
  async function setupSiteFooter(){
    ...
    const version=document.createElement('span');version.className='ranhados-external-footer__version';version.textContent='Meteo Ranhados v4.0.0-alpha2 · 26/09/2026';
    ...
    const contact=document.createElement('a');contact.className='ranhados-external-footer__contact';contact.href='/contacto/';contact.textContent='Contacto / sugerir correção';
    meta.append(version,label,contact);
    ...
    try{
      const r=await fetch('/site-meta.json',{cache:'no-store'});if(r.ok){const m=await r.json();const d=m.release_date?new Date(m.release_date+'T12:00:00'):null;const ds=d&&!Number.isNaN(d.getTime())?d.toLocaleDateString('pt-PT')  :'23/09/2026';const gd=m.generated_utc?new Date(m.generated_utc):null,gs=gd&&!Number.isNaN(gd.getTime())?gd.toLocaleString('pt-PT',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}):'';version.textContent=`Meteo Ranhados v${m.version||'4.0.0-alpha2'} · ${ds}${gs?' · gerado '+gs:''}`}
    }catch(e){}
    try{
  ...
  function setupServiceWorker(){
    ...
    navigator.serviceWorker.register('/sw.js?v=4.0.0-alpha2',{scope:'/'}).then(reg=>{reg.update().catch(()=>{});
