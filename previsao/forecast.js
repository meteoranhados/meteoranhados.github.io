...
function renderMeteogram(){
 ...
 s+=`<line id="fx-cross" x1="0" y1="${m.t}" x2="0" y2="${H-m.b}" stroke="#72828b" stroke-dasharray="3 4" visibility="hidden"/><rect id="fx-hit" x="${m.l}" y="${m.t}" width="${W-m.l-m.r}" height="${H-m.t-m.b}" fill="transparent"/></svg>`;const chart=$('#fx-chart');chart.innerHTML=s+'<div id="fx-tip" class="fx-tooltip"></div>';const svg=chart.querySelector('svg'),hit=chart.querySelector('#fx-hit'),cross=chart.querySelector('#fx-cross'),tip=chart.querySelector('#fx-tip');hit.addEventListener('mousemove',ev=>{const rect=svg.getBoundingClientRect(),vx=(ev.clientX-rect.left)/rect.width*W,target=minT+(vx-m.l)/(W-m.l-m.r)*(maxT-minT),r=rows.reduce((a,b)=>Math.abs(new Date(b.time).getTime()-target)<Math.abs(new Date(a.time).getTime()-target)?b:a,rows[0]),xx=x(new Date(r.time).getTime());cross.setAttribute('x1',xx);cross.setAttribute('x2',xx);cross.setAttribute('visibility','visible');let lines='';if(tab==='rain'){lines+=`<div><span>Precipitação</span><b>${fmt(r.precipitation,1)} mm</b></div><div><span>Probabilidade</span><b>${fmt(r.precipitation_probability,0)}%</b></div>`}else defs.forEach(d=>{if(r[d[0]]!=null)lines+=`<div><span>${d[1]}</span><b>${fmt(r[d[0]],tab==='humidity'?0:1)} ${cfg.unit}</b></div>`});if(tab==='temperature'&&forecast?.local_bridge?.available){const all=forecast.primary?.hourly?.time||[],i=all.indexOf(r.time),v=forecast.local_bridge.hourly?.temperature_2m?.[i];if(v!=null)lines+=`<div><span>Ponte local</span><b>${fmt(v,1)} °C</b></div>`}tip.innerHTML=`<strong>${dtFmt(r.time)} · ${timeFmt(r.time)}</strong>${lines}`;tip.style.display='block';tip.style.left=Math.min(rect.width-190,Math.max(5,ev.clientX-rect.left+12))+'px';tip.style.top=Math.max(8,ev.clientY-rect.top-40)+'px'});hit.addEventListener('mouseleave',()=>{cross.setAttribute('visibility','hidden');tip.style.display='none'});
 let cards=[];
...
function renderAgro(){
 const daily=dailyRows(forecast?.primary).slice(0,7),hour=future72(),rain=daily.reduce((a,r)=>a+Number(r.precipitation_sum||0),0),et=daily.reduce((a,r)=>a+Number(r.et0_fao_evapotranspiration||0),0),balance=rain-et,maxVpd=Math.max(0,...hour.map(r=>Number(r.vapour_pressure_deficit||0))),min3=Math.min(...daily.slice(0,3).map(r=>Number(r.temperature_2m_min)).filter(Number.isFinite)),soil=hour.find(r=>r.soil_moisture_0_to_1cm!=null)?.soil_moisture_0_to_1cm;
 const frost=Number.isFinite(min3)&&min3<=1?'atenção a frio/geada':'sem sinal térmico forte';
 const items=[['Chuva 7 dias',`${fmt(rain,1)} mm`,'previsão'],['ET₀ 7 dias',`${fmt(et,1)} mm`,'FAO-56'],['Balanço simples',`${balance>=0?'+':''}${fmt(balance,1)} mm`,'chuva − ET₀'],['VPD máximo 72 h',`${fmt(maxVpd,2)} kPa`,'stress atmosférico'],['Frio 3 noites',Number.isFinite(min3)?`${fmt(min3,1)} °C`:'—',frost],['Humidade solo superficial',soil!=null?`${fmt(soil,3)} m³/m³`:'—','valor modelado']];
