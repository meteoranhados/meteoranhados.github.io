...
function weatherSymbol(code,isDay=1){return window.MeteoWeather?.icon?window.MeteoWeather.icon(code,isDay):'—'}
function weatherText(code){return window.MeteoWeather?.text?window.MeteoWeather.text(code):'Tempo variável'}
...
function renderDays(){
 ...
 $('#fx-days').innerHTML=rows.map((r,i)=>`<article class="fx-day ${i===0?'today':''} ${i>=7?'is-long-range':i>=4?'is-mid-range':'is-near-range'}"><b>${i===0?'Hoje':dayFmt(r.time).split(',')[0]}</b><span class="date">${dayFmt(r.time).replace(/^[^,]+,\s*/,'')}</span><div class="symbol">${weatherSymbol(r.weather_code,1)}<em>${weatherText(r.weather_code)}</em></div><span class="temp">${fmt(r.temperature_2m_max,0)}° <small style="display:inline">${fmt(r.temperature_2m_min,0)}°</small></span><small class="fx-meta-line">${window.MeteoWeather?.ui('rain')||''}${fmt(r.precipitation_probability_max,0)}% · ${fmt(r.precipitation_sum,1)} mm</small><small class="fx-meta-line">${window.MeteoWeather?.ui('wind')||''}${windDir(r.wind_direction_10m_dominant)} · ${fmt(r.wind_gusts_10m_max,0)} km/h</small><small>UV ${fmt(r.uv_index_max,0)}</small></article>`).join('');
}
...
function modelDay(model,date){return dailyRows(model).find(r=>r.time===date)}
function ensembleSpreadForDate(date){const h=forecast?.ensemble?.hourly||{},times=h.time||[],vals=[];times.forEach((t,i)=>{if(String(t).slice(0,10)===date){const v=Number(h.temperature_2m_spread?.[i]);if(Number.isFinite(v))vals.push(v)}});return vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null}
function confidenceFor(day){const e=modelDay(forecast?.models?.ecmwf,day.time),g=modelDay(forecast?.models?.gfs,day.time),factors=[];if(e&&g&&e.temperature_2m_max!=null&&g.temperature_2m_max!=null){const v=Math.abs(Number(e.temperature_2m_max)-Number(g.temperature_2m_max));factors.push(v<1.5?0:v<3?1:2)}if(e&&g&&e.precipitation_sum!=null&&g.precipitation_sum!=null){const v=Math.abs(Number(e.precipitation_sum)-Number(g.precipitation_sum));factors.push(v<2?0:v<7?1:2)}const sp=ensembleSpreadForDate(day.time);if(sp!=null)factors.push(sp<1.5?0:sp<2.5?1:2);if(!factors.length)return{label:'Sem avaliação',cls:'unknown',note:'sem dispersão suficiente'};const avg=factors.reduce((a,b)=>a+b,0)/factors.length;return avg<.65?{label:'Alta',cls:'high',note:'modelos próximos'}:avg<1.35?{label:'Moderada',cls:'medium',note:'alguma dispersão'}:{label:'Baixa',cls:'low',note:'divergência relevante'}}
function trendItem(label,v,unit,threshold,d=1){
 const x=Number(v);if(!Number.isFinite(x)||Math.abs(x)<threshold)return null;
 const up=x>0;
 return {text:`${label} ${up?'subiu':'desceu'} ${fmt(Math.abs(x),d)} ${unit}`,dir:up?'up':'down'};
}
function weatherChangeItems(ed){
 const items=[
  trendItem('Máxima',ed.temperature_2m_max,'°C',0.5,1),
  trendItem('Prob. de chuva',ed.precipitation_probability_max,'p.p.',10,0),
  trendItem('Precipitação',ed.precipitation_sum,'mm',0.5,1),
  trendItem('Rajada máx.',ed.wind_gusts_10m_max,'km/h',5,0)
 ].filter(Boolean);
 if(ed.weather_code_changed)items.push({text:'condição meteorológica alterada',dir:'weather'});
 return items;
}
function renderBrief(){
 const rows=dailyRows(forecast?.primary).slice(0,3),host=$('#fx-brief-cards'),change=$('#fx-change');
 if(!host)return;
 if(!rows.length){host.innerHTML='<div class="fx-empty"><strong>Resumo indisponível</strong><span>A previsão principal não devolveu dias suficientes.</span></div>';return}
 host.innerHTML=rows.map((r,i)=>{
   const c=confidenceFor(r),label=i===0?'Hoje':i===1?'Amanhã':dayFmt(r.time).split(',')[0];
   return `<article class="fx-brief-card"><div class="fx-brief-top"><div>${weatherSymbol(r.weather_code,1)}</div><span>${label}<small>${weatherText(r.weather_code)}</small></span><i class="fx-confidence is-${c.cls}">Confiança ${c.label}</i></div><div class="fx-brief-temp"><b>${fmt(r.temperature_2m_max,0)}°</b><span>mín. ${fmt(r.temperature_2m_min,0)}°</span></div><div class="fx-brief-meta"><span>${window.MeteoWeather?.ui('rain')||''}${fmt(r.precipitation_probability_max,0)}% · ${fmt(r.precipitation_sum,1)} mm</span><span>${window.MeteoWeather?.ui('wind')||''}rajada ${fmt(r.wind_gusts_10m_max,0)} km/h</span></div><small class="fx-confidence-note">${c.note}</small></article>`
 }).join('');
 const target=rows[1]||rows[0],ev=forecast?.evolution,ed=ev?.days?.[target.time],label=rows[1]?'amanhã':dayFmt(target.time);
 if(ev?.available&&ed){
   const items=weatherChangeItems(ed);
   if(items.length){
     change.innerHTML=`<div class="fx-change-head"><strong>Como evoluiu a previsão de ${label}?</strong><small>face à emissão de há ${fmt(ev.hours_ago,0)} h</small></div><div class="fx-change-items">${items.map(x=>`<span class="is-${x.dir}">${x.dir==='up'?'↑':x.dir==='down'?'↓':'↺'} ${x.text}</span>`).join('')}</div><p>Mostra estabilidade da previsão; não mede o acerto face ao observado.</p>`
   }else{
     change.innerHTML=`<div class="fx-change-head"><strong>Previsão de ${label} praticamente estável</strong><small>face à emissão de há ${fmt(ev.hours_ago,0)} h</small></div><div class="fx-change-stable">Sem alterações relevantes na máxima, chuva ou rajada.</div><p>Pequenas oscilações são omitidas para evitar ruído.</p>`
   }
 }else{
   change.innerHTML=`<div class="fx-change-head"><strong>Evolução da previsão</strong><small>histórico em preparação</small></div><div class="fx-change-stable">${ev?.message||'A acumular histórico suficiente para comparar emissões.'}</div>`
 }
}
...
function renderMeteogram(){
 const rows=future72(),cfg=TABS?.[tab],chart=$('#fx-chart'),statsHost=$('#fx-meteo-stats'),note=$('#fx-chart-note');if(!cfg)throw new Error('Configuração do meteograma em falta: '+tab);
...
function renderHourly(){
 const rows=future72().slice(0,24);$('#fx-hourly-table').innerHTML=`<table><thead><tr><th>Hora</th><th>Tempo</th><th>Temperatura</th><th>Humidade</th><th>Chuva</th><th>Prob.</th><th>Vento</th><th>Rajada</th><th>Pressão</th><th>Nuvens</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${dtFmt(r.time)}</td><td><span class="fx-hourly-wx">${weatherSymbol(r.weather_code,r.is_day??1)}<small>${weatherText(r.weather_code)}</small></span></td><td>${fmt(r.temperature_2m,1)} °C</td><td>${fmt(r.relative_humidity_2m,0)}%</td><td>${fmt(r.precipitation,1)} mm</td><td>${fmt(r.precipitation_probability,0)}%</td><td>${windDir(r.wind_direction_10m)} ${fmt(r.wind_speed_10m,0)}</td><td>${fmt(r.wind_gusts_10m,0)} km/h</td><td>${fmt(r.pressure_msl,0)} hPa</td><td>${fmt(r.cloud_cover,0)}%</td></tr>`).join('')}</tbody></table>`;
}
async function init(){
