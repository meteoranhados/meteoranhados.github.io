...
function weatherIcon(code,isDay=1){return window.MeteoWeather?.icon?window.MeteoWeather.icon(code,isDay):icon('cloud')}
function weatherText(code){return window.MeteoWeather?.text?window.MeteoWeather.text(code):'Condições variáveis'}
function nearestForecastIndex(times){const now=Date.now();let best=0,delta=Infinity;(times||[]).forEach((t,i)=>{const d=Math.abs(new Date(t).getTime()-now);if(Number.isFinite(d)&&d<delta){delta=d;best=i}});return best}
...
function renderCurrent(){
  ...
  const wxIndex=nearestForecastIndex(primary.time||[]),code=primary.weather_code?.[wxIndex]??0,isDay=primary.is_day?.[wxIndex]??1;
  $('#now-symbol').innerHTML=weatherIcon(code,isDay);
...
function renderForecast(){
  ...
  $('#daily-forecast').innerHTML=rows.map((r,i)=>{
    ...
    return `<article class="forecast-day ${i===0?'today':''}">
      ...
      <div class="wx">${weatherIcon(r.weather_code,1)}<small>${weatherText(r.weather_code)}</small></div>
