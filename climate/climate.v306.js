...
function shortDate(s){
  if(!s)return '—';
  return new Date(s+'T12:00:00').toLocaleDateString('pt-PT',{day:'2-digit',month:'short'}).replaceAll('.','');
}
function monthWaveEpisodes(waves,year,month,kind){
  const arr=kind==='heat'?(waves?.heatwaves||[]):(waves?.coldwaves||[]);
  const prefix=`${year}-${String(month).padStart(2,'0')}-`;
  return arr.filter(ep=>(ep.daily||[]).some(x=>String(x.date||'').startsWith(prefix)));
}
function waveOverviewCard(kind,status,waves,daily,lastClosed,live,liveStatus,year,month){
  const heat=kind==='heat',streak=Number(status?.qualifying_streak_days||0);
  const need=Number(status?.days_needed_for_confirmation||0),minDays=Math.max(1,streak+need||6);
  const active=status?.active===true;
  const state=active?'Ativa':streak>=3?'Em formação':streak>0?'Em observação':'Não ativa';
  const closed=(daily?.days||[]).find(x=>x.date===lastClosed)||{};
  const threshold=Number(heat?closed.heat_threshold_c:closed.cold_threshold_c);
  const thresholdLabel=Number.isFinite(threshold)?`${heat?'Tmax >':'Tmin <'} ${n(threshold,2)} °C`:'Limiar indisponível';
  const episodes=monthWaveEpisodes(waves,year,month,kind);
  const lastEpisode=episodes.at(-1);
  let currentLine='Sem sequência no último dia fechado.';
  if(streak){
    const range=status?.start&&status?.last_date?`${shortDate(status.start)}–${shortDate(status.last_date)}`:shortDate(status?.last_date);
    currentLine=`${range} · <strong>${streak} dia${streak===1?'':'s'}</strong>`;
    if(!active&&need>0)currentLine+=` · faltam ${need}`;
  }
  let monthLine=`${ml[month]}: sem ondas ${heat?'de calor':'de frio'} confirmadas.`;
  if(episodes.length){
    const ep=lastEpisode;
    monthLine=`${ml[month]}: <strong>${episodes.length}</strong> ${episodes.length===1?'onda confirmada':'ondas confirmadas'} · última ${shortDate(ep.start)}–${shortDate(ep.end)} (${ep.days} dias).`;
  }
  let liveLine='';
  if(live&&!live.error&&live.date&&String(live.date)>String(lastClosed)&&Number.isFinite(threshold)){
    const lv=Number(heat?live.tmax:live.tmin);
    if(Number.isFinite(lv)){
      const qualifies=heat?lv>threshold:lv<threshold;
      const preview=Number(liveStatus?.qualifying_streak_days||0);
      liveLine=qualifies
        ?`Hoje, provisoriamente, também cumpre o limiar (${n(lv,1)} °C)${preview>streak?`; se fechar assim, serão ${preview} dias`:''}.`
        :`Hoje está em curso (${n(lv,1)} °C até agora); um dia incompleto não apaga a sequência fechada.`;
    }
  }
  const progress=Math.min(100,Math.max(0,streak/minDays*100));
  return `<div class="wave-card ${heat?'heat':'cold'} ${active?'is-active':streak>=3?'is-forming':''}">
    <div class="wave-card-head"><span>Onda ${heat?'de calor':'de frio'}</span><em>${state}</em></div>
    <div class="wave-card-main"><b>${streak}</b><span>dia${streak===1?'':'s'} qualificativo${streak===1?'':'s'}</span></div>
    <div class="wave-progress" role="progressbar" aria-valuemin="0" aria-valuemax="${minDays}" aria-valuenow="${Math.min(streak,minDays)}"><i style="width:${progress}%"></i></div>
    <div class="wave-card-rule">${thresholdLabel} · confirmação aos ${minDays} dias</div>
    <div class="wave-card-current">${currentLine}</div>
    <div class="wave-card-history">${monthLine}</div>
    ${liveLine?`<div class="wave-card-live">${liveLine}</div>`:''}
  </div>`;
}
...
function interactiveLineChart(el,series,opt={}){
  ...
  const draw=()=>{
    ...
    const yAxisLabel=(opt.yLabel||opt.axisLabel||String(opt.unit||'').trim());
    if(yAxisLabel)svg.append(se('text',{transform:`translate(17 ${H/2}) rotate(-90)`,'text-anchor':'middle','font-size':11,'font-weight':750,fill:'#64727d'},yAxisLabel));
...
function interactiveGroupedBars(el,categories,series,opt={}){
  ...
  const draw=()=>{
    ...
    const yAxisLabel=(opt.yLabel||opt.axisLabel||String(opt.unit||'').trim());if(yAxisLabel)svg.append(se('text',{transform:`translate(17 ${H/2}) rotate(-90)`,'text-anchor':'middle','font-size':11,'font-weight':750,fill:'#64727d'},yAxisLabel));
...
function multiAnnualIndexChart(el,rows,metrics,selectedKeys,mode){
  ...
  svg.append(se('text',{transform:`translate(17 ${H/2}) rotate(-90)`,'text-anchor':'middle','font-size':11,'font-weight':750,fill:'#64727d'},'Dias'));
...
function annualBarChart(el,rows,metric,opt={}){
  ...
  const yAxisLabel=metric.axisLabel||metric.label||String(metric.unit||'').trim();if(yAxisLabel)svg.append(se('text',{transform:`translate(17 ${H/2}) rotate(-90)`,'text-anchor':'middle','font-size':11,'font-weight':750,fill:'#64727d'},yAxisLabel));
...
async function init(){
  ...
  const tempWord=Number(cm.anomaly?.tmean_c)>0.5?'mais quente':Number(cm.anomaly?.tmean_c)<-0.5?'mais frio':'próximo da normal',rainWord=Number(cm.anomaly?.precip_pct_normal)>120?'mais chuvoso':Number(cm.anomaly?.precip_pct_normal)<80?'mais seco':'próximo da normal';
  $('#overview-insight').innerHTML=`<div class="insight-label">${cm.month_name} em contexto</div><div class="insight-main">Um mês <strong>${tempWord}</strong> e <strong>${rainWord}</strong> do que a referência climatológica.</div><div class="overview-closed-day">Dados observados fechados até <strong>${dpt(s.source_last_closed_day)}</strong></div><div class="insight-metrics"><span><b>${n(cm.observed?.tmean_c,2)} °C</b><small>T média observada</small></span><span><b>${anomalyHtml(cm.anomaly?.tmean_c,2)}</b><small>vs normal · ${anomalyHtml(monthTempStation,2)} vs estação</small></span><span><b>${n(cm.observed?.precip_mm,1)} mm</b><small>precipitação observada</small></span><span><b>${n(cm.anomaly?.precip_pct_normal,0)}%</b><small>normal · ${n(monthRainStationPct,0)}% estação</small></span></div><span class="insight-sub">${y.year}: T média <strong>${n(y.observed?.tmean_c,2)} °C</strong> · ${anomalyHtml(y.anomaly?.tmean_c,2)} vs normal e ${anomalyHtml(sd.tmean_c,2)} vs estação · precipitação <strong>${n(y.observed?.precip_mm,1)} mm</strong> · ${n(y.anomaly?.precip_pct_normal,0)}% da normal.</span>`;
  const heatLive=s.wave_live_preview?.heatwave||s.heatwave,coldLive=s.wave_live_preview?.coldwave||s.coldwave;
  $('#hero').innerHTML=`<div class="overview-metric-card thermal"><div class="metric-card-label">${cm.month_name} · desvio térmico</div><b>${anomalyHtml(cm.anomaly?.tmean_c,2)}</b><span>${n(cm.observed?.tmean_c,2)} °C de média observada</span><small>${anomalyHtml(monthTempStation,2)} vs média dos anos anteriores da estação</small></div><div class="overview-metric-card rainfall"><div class="metric-card-label">Precipitação no mês</div><b>${n(cm.observed?.precip_mm,1)} mm</b><span>${n(cm.anomaly?.precip_pct_normal,0)}% da normal 1991–2020</span><small>${n(monthRainStationPct,0)}% da média dos anos anteriores da estação</small></div>${waveOverviewCard('heat',s.heatwave,w,d,s.source_last_closed_day,live,heatLive,y.year,cm.month)}${waveOverviewCard('cold',s.coldwave,w,d,s.source_last_closed_day,live,coldLive,y.year,cm.month)}`;
