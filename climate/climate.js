...
async function init(){
  ...
  const [s,m,h,q,w,e,d,st,ind,rec,ranks,annual,ac]=await Promise.all(names.map(fetchJson));
  ...
  const hp=hy.cumulative_points||[];interactiveLineChart($('#hydro-chart'),[{label:'Observado',data:hp.map(p=>({label:p.month_name,value:p.observed_cumulative_mm})),unit:' mm',color:cssv('--rain-observed')},{label:'Normal 1991–2020',data:hp.map(p=>({label:p.month_name,value:p.normal_cumulative_mm})),unit:' mm',color:cssv('--rain-normal')}],{unit:' mm',zeroMin:true});
