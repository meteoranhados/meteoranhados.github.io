...
function metricColor(key){const map={tmax_ge_25:'#6f8f61',tmax_ge_30:'#c68a3b',tmax_gt_35:'#b74e32',tmax_ge_40:'#8d3f32',tropical_nights:'#667ea8',tmin_le_0:'#4d7eac',frost_days:'#365f8d'};return map[key]||cssv('--accent')}
...
function multiAnnualIndexChart(el,rows,metrics,selectedKeys,mode){
  ...
  if(!selected.length||!filtered.length){el.innerHTML='<div class="chart-empty">Selecione pelo menos um indicador.</div>';return}
  ...
  filtered.forEach((r,i)=>{const center=m.l+(i+.5)*group;svg.append(se('text',{x:center,y:H-28,'text-anchor':'middle','font-size':11,fill:'#64727d'},String(r.label||r.year)));selected.forEach(([key,def],j)=>{const v=Number(r.observed?.[key]);if(!Number.isFinite(v))return;const quality=mode==='same'?r.quality?.temperature_valid:r.annual_quality?.temperature_year_valid,x=center-(selected.length*bw)/2+j*bw,yy=y(v),rect=se('rect',{x,y:yy,width:bw-3,height:H-m.b-yy,rx:4,fill:quality?metricColor(key):'#fff',stroke:quality?metricColor(key):metricColor(key),'stroke-width':quality?1:3,opacity:quality?0.94:1,style:'cursor:pointer'});rect.addEventListener('mousemove',ev=>{let html=`<b>${r.label||r.year}</b><div class="tip-row"><span>${def.label}</span><strong>${n(v,0)} dias</strong></div>`;const normal=def.normal_available?r.normal_1991_2020_same_period?.[key]:null;if(Number.isFinite(Number(normal)))html+=`<div class="tip-row"><span>Normal comparável</span><strong>${n(normal,1)} dias</strong></div>`;html+=`<div class="tip-row"><span>Cobertura</span><strong>${n(r.coverage_pct,1)}%</strong></div><div class="tip-note">${quality?'Período válido.':'Valor mostrado com ressalva de qualidade.'}</div>`;showTip(ev,html)});rect.addEventListener('mouseleave',hideTip);svg.append(rect)})});
...
function annualBarChart(el,rows,metric,opt={}){
  ...
  filtered.forEach((r,i)=>{
    ...
    const semantic=metric.semantic==='temperature_anomaly'?anomalyColor(v):(metric.color||cssv('--single-series'));
    const rect=se('rect',{x,y:yy,width:bw,height:hh,rx:5,fill:quality?semantic:'#fff',stroke:semantic,'stroke-width':quality?1:3,style:'cursor:pointer'});
  ...
  el.append(svg);
  if(metric.semantic==='temperature_anomaly'){
    const leg=document.createElement('div');leg.className='semantic-legend';leg.innerHTML='<span><i style="background:var(--anom-hot)"></i>anomalia positiva</span><span><i style="background:var(--anom-cold)"></i>anomalia negativa</span>';el.append(leg);
  }
...
async function init(){
  ...
  function buildThermalPicker(){const host=$('#thermal-index-picker');host.innerHTML='';thermalDefs.forEach(k=>{const def=ind.definitions[k],lab=document.createElement('label');lab.className='index-choice';const cb=document.createElement('input');cb.type='checkbox';cb.value=k;cb.checked=thermalState.selected.has(k);cb.addEventListener('change',()=>{if(cb.checked&&thermalState.selected.size>=3){cb.checked=false;const note=$('#thermal-index-note');note.textContent='Pode comparar até três indicadores em simultâneo.';note.classList.add('pos');setTimeout(()=>note.classList.remove('pos'),1200);return}if(cb.checked)thermalState.selected.add(k);else{if(thermalState.selected.size===1){cb.checked=true;return}thermalState.selected.delete(k)}renderThermalAnnual()});lab.append(cb,document.createTextNode(def.label));host.append(lab)});const lim=document.createElement('span');lim.className='index-limit-note';lim.textContent='máx. 3 séries';host.append(lim)}
  function renderThermalAnnual(){const mode=$('#thermal-period').value,rows=mode==='same'?ac.same_period:ac.full_years,keys=[...thermalState.selected],defs=Object.fromEntries(keys.map(k=>[k,ind.definitions[k]]));$('#thermal-index-note').textContent=`${keys.length} indicador${keys.length===1?'':'es'} selecionado${keys.length===1?'':'s'}. As barras vazias assinalam períodos com ressalva; a normal compatível aparece no tooltip quando existe.`;multiAnnualIndexChart($('#thermal-index-chart'),rows,defs,keys,mode)}
  ...
  annualBarChart($('#annual-anomaly-chart'),annualSame,{label:'Anomalia da T média',unit:' °C',decimals:2,symmetric:true,semantic:'temperature_anomaly',value:r=>r.anomaly?.tmean_c,normal:null,quality:r=>r.quality?.temperature_valid});
  $('#annual-rain-note').textContent=`1 Jan–${ac.latest_month_day}, precipitação observada em percentagem da normal 1991–2020.`;
  annualBarChart($('#annual-rain-pct-chart'),annualSame,{label:'Precipitação em % da normal',unit:'%',decimals:0,color:cssv('--wet'),referenceLine:100,referenceColor:cssv('--rain-normal'),value:r=>r.anomaly?.precip_pct_normal,normal:r=>100,quality:r=>r.quality?.precipitation_valid});

  // HISTÓRICO / COMPARAÇÃO ANUAL
  const annualMetrics={tmean_anom:{label:'Anomalia da T média',unit:' °C',decimals:2,symmetric:true,semantic:'temperature_anomaly',value:r=>r.anomaly?.tmean_c,normal:null,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},tmax:{label:'Tmax média',unit:' °C',decimals:2,color:cssv('--single-series'),value:r=>r.observed?.tmax_mean_c,normal:r=>r.normal_1991_2020_same_period?.tmax_mean_c,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},tmin:{label:'Tmin média',unit:' °C',decimals:2,color:cssv('--single-series'),value:r=>r.observed?.tmin_mean_c,normal:r=>r.normal_1991_2020_same_period?.tmin_mean_c,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},precip_pct:{label:'Precipitação em % da normal',unit:'%',decimals:0,color:cssv('--wet'),referenceLine:100,referenceColor:cssv('--rain-normal'),value:r=>r.anomaly?.precip_pct_normal,normal:r=>100,quality:r=>r.quality?r.quality.precipitation_valid:r.annual_quality?.precipitation_year_valid},hot30:{label:'Dias ≥30 °C',unit:' dias',decimals:0,color:cssv('--warm'),value:r=>r.observed?.tmax_ge_30,normal:null,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},hot35:{label:'Dias >35 °C',unit:' dias',decimals:0,color:cssv('--warm'),value:r=>r.observed?.tmax_gt_35,normal:r=>r.normal_1991_2020_same_period?.tmax_gt_35,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},tropical:{label:'Noites tropicais',unit:' dias',decimals:0,color:cssv('--cold'),value:r=>r.observed?.tropical_nights,normal:r=>r.normal_1991_2020_same_period?.tropical_nights,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},rain_days:{label:'Dias ≥1 mm',unit:' dias',decimals:0,color:cssv('--wet'),value:r=>r.observed?.rain_days,normal:r=>r.normal_1991_2020_same_period?.rain_days,quality:r=>r.quality?r.quality.precipitation_valid:r.annual_quality?.precipitation_year_valid},heatwave:{label:'Dias em ondas de calor',unit:' dias',decimals:0,color:cssv('--warm'),value:r=>r.observed?.heatwave_days,normal:r=>r.normal_1991_2020_same_period?.heatwave_days,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid}};
