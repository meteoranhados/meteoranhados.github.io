...
async function init(){
  ...
  $('#overview-insight').innerHTML=`<div class="insight-main"><strong>${cm.month_name}:</strong> T média ${anomalyHtml(cm.anomaly?.tmean_c,2)} face à normal e ${anomalyHtml(monthTempStation,2)} face à média da estação para os mesmos dias.</div><span class="insight-sub">Em ${y.year}, a T média está ${anomalyHtml(y.anomaly?.tmean_c,2)} vs normal e ${anomalyHtml(sd.tmean_c,2)} vs média da estação; precipitação em ${n(y.anomaly?.precip_pct_normal,0)}% da normal e ${n(sd.precip_pct_reference,0)}% da média da estação.</span>`;
  ...
  const hydroMetrics={pct:{label:'Precipitação em % da normal',unit:'%',decimals:0,color:cssv('--rain-observed'),referenceLine:100,referenceColor:cssv('--rain-normal'),value:r=>r.anomaly?.precip_pct_normal,normal:r=>100},total:{label:'Precipitação acumulada',unit:' mm',decimals:1,color:cssv('--rain-observed'),value:r=>r.observed?.precip_mm,normal:r=>r.normal_1991_2020_same_period?.precip_mm}};
  function renderHydroHistory(){const mode=$('#hydro-period').value,metric=hydroMetrics[$('#hydro-metric').value],rows=mode==='same'?hc.same_period:hc.full_years;$('#hydro-history-note').textContent=mode==='same'?`${hc.same_period_label}: todos os anos hidrológicos na mesma fase do ciclo.`:'Apenas anos hidrológicos completos; contorno dourado = existem dias em falta.';annualBarChart($('#hydro-history-chart'),rows,{...metric,quality:r=>r.quality?.precipitation_valid})}
  ...
  const annualMetrics={tmean_anom:{label:'Anomalia da T média',unit:' °C',decimals:2,symmetric:true,semantic:'temperature_anomaly',value:r=>r.anomaly?.tmean_c,normal:null,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},tmax:{label:'Tmax média',unit:' °C',decimals:2,color:cssv('--single-series'),value:r=>r.observed?.tmax_mean_c,normal:r=>r.normal_1991_2020_same_period?.tmax_mean_c,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},tmin:{label:'Tmin média',unit:' °C',decimals:2,color:cssv('--single-series'),value:r=>r.observed?.tmin_mean_c,normal:r=>r.normal_1991_2020_same_period?.tmin_mean_c,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},precip_pct:{label:'Precipitação em % da normal',unit:'%',decimals:0,color:'#3978a6',referenceLine:100,referenceColor:cssv('--rain-normal'),value:r=>r.anomaly?.precip_pct_normal,normal:r=>100,quality:r=>r.quality?r.quality.precipitation_valid:r.annual_quality?.precipitation_year_valid},hot25:{label:'Dias com Tmax ≥25 °C',unit:' dias',decimals:0,color:'#e5a092',value:r=>r.observed?.tmax_ge_25,normal:null,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},hot30:{label:'Dias com Tmax ≥30 °C',unit:' dias',decimals:0,color:'#d9725e',value:r=>r.observed?.tmax_ge_30,normal:null,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},hot35:{label:'Dias com Tmax >35 °C',unit:' dias',decimals:0,color:'#bd4a3e',value:r=>r.observed?.tmax_gt_35,normal:r=>r.normal_1991_2020_same_period?.tmax_gt_35,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},hot40:{label:'Dias com Tmax ≥40 °C',unit:' dias',decimals:0,color:'#8f3038',value:r=>r.observed?.tmax_ge_40,normal:null,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},tropical:{label:'Dias com Tmin ≥20 °C',unit:' dias',decimals:0,color:'#c45b72',value:r=>r.observed?.tropical_nights,normal:r=>r.normal_1991_2020_same_period?.tropical_nights,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},coldle0:{label:'Dias com Tmin ≤0 °C',unit:' dias',decimals:0,color:'#4e7fb4',value:r=>r.observed?.tmin_le_0,normal:null,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},coldlt0:{label:'Dias com Tmin <0 °C',unit:' dias',decimals:0,color:'#2f6198',value:r=>r.observed?.frost_days,normal:r=>r.normal_1991_2020_same_period?.frost_days,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},rain_days:{label:'Dias com precipitação ≥1 mm',unit:' dias',decimals:0,color:'#5f9a70',value:r=>r.observed?.rain_days,normal:r=>r.normal_1991_2020_same_period?.rain_days,quality:r=>r.quality?r.quality.precipitation_valid:r.annual_quality?.precipitation_year_valid},rain10:{label:'Dias com precipitação >10 mm',unit:' dias',decimals:0,color:'#d1aa3f',value:r=>r.observed?.rain_gt_10,normal:r=>r.normal_1991_2020_same_period?.rain_gt_10,quality:r=>r.quality?r.quality.precipitation_valid:r.annual_quality?.precipitation_year_valid},rain20:{label:'Dias com precipitação >20 mm',unit:' dias',decimals:0,color:'#4b9a8f',value:r=>r.observed?.rain_gt_20,normal:r=>r.normal_1991_2020_same_period?.rain_gt_20,quality:r=>r.quality?r.quality.precipitation_valid:r.annual_quality?.precipitation_year_valid},rain30:{label:'Dias com precipitação >30 mm',unit:' dias',decimals:0,color:'#3978a6',value:r=>r.observed?.rain_gt_30,normal:r=>r.normal_1991_2020_same_period?.rain_gt_30,quality:r=>r.quality?r.quality.precipitation_valid:r.annual_quality?.precipitation_year_valid},heatwave:{label:'Dias em ondas de calor',unit:' dias',decimals:0,color:cssv('--warm'),value:r=>r.observed?.heatwave_days,normal:r=>r.normal_1991_2020_same_period?.heatwave_days,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid},coldwave:{label:'Dias em ondas de frio',unit:' dias',decimals:0,color:cssv('--cold'),value:r=>r.observed?.coldwave_days,normal:r=>r.normal_1991_2020_same_period?.coldwave_days,quality:r=>r.quality?r.quality.temperature_valid:r.annual_quality?.temperature_year_valid}};
  Object.entries(annualMetrics).forEach(([k,v])=>$('#annual-metric').add(new Option(v.label,k)));$('#annual-metric').value='tmean_anom';function renderAnnualCompare(){const mode=$('#annual-period').value,metric=annualMetrics[$('#annual-metric').value],rows=mode==='same'?ac.same_period:ac.full_years;$('#annual-comparison-note').textContent=mode==='same'?`Todos os anos de 1 Jan até ${ac.latest_month_day}; evita comparar ${y.year} parcial com anos inteiros.`:'Apenas anos de calendário fechados; um contorno dourado assinala qualidade insuficiente para a métrica.';annualBarChart($('#annual-comparison-chart'),rows,metric)}$('#annual-period').addEventListener('change',renderAnnualCompare);$('#annual-metric').addEventListener('change',renderAnnualCompare);renderAnnualCompare();
