...
async function init(){
 ...
 try{
  ...
  const lab=d.laboratory||{};$('#sx-lab').innerHTML=row('MRL local',lab.local_model_available?(lab.model_id||'Disponível'):'Indisponível')+row('Melhoria média validação',lab.validation_improvement_pct!=null?`${Number(lab.validation_improvement_pct)>=0?'+':''}${fmt(lab.validation_improvement_pct,1)}%`:'—')+row('Verificação geral',lab.verification_available?`${lab.verification_samples||0} amostras`:'A recolher')+row('Arquivo MRL',`${lab.mrl_verified||0} verificadas · ${lab.mrl_pending||0} pendentes`);
