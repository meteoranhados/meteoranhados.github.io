...
async function loadPeriod(p){
 ...
 try{
   ...
   else{data=await getJson('daily-archive.json');let rows=data.samples||[];if(p==='custom'){const from=$('#gx-from').value,to=$('#gx-to').value,lo=from?new Date(from+'T00:00:00').getTime():-Infinity,hi=to?new Date(to+'T23:59:59').getTime():Infinity;rows=rows.filter(x=>x.time_ms>=lo&&x.time_ms<=hi)}else rows=rowsForDailyPeriod(rows,p);data={...data,samples:rows}}
...
function setupCompare(){
 ...
 [...sel.options].forEach(o=>{
   if(!o.value)return;
   const map=daily?{hum:'hum_mid',pressure:'press_mid',wind_avg:'wind_avg_max',dew:null,rain_mm:'rain_mm',solar:'solar_max_day'}:null;
...
function compareActualKey(){
 ...
 return {hum:'hum_mid',pressure:'press_mid',wind_avg:'wind_avg_max',dew:null,rain_mm:'rain_mm',solar:'solar_max_day'}[compareKey]||'';
}
...
function renderChart(){
 ...
 const tickMap=mobile?{'24h':6,'7d':7,'30d':7,'90d':6,'year':6,'all':6,'custom':6}:{'24h':12,'7d':14,'30d':15,'90d':10,'year':12,'all':10,'custom':10};
 ...
 const labelFor=t=>{
   ...
   if(period==='custom'&&(maxT-minT)>180*86400000)return new Intl.DateTimeFormat('pt-PT',{month:'2-digit',year:'2-digit'}).format(d);
 ...
 hit.addEventListener('pointermove',ev=>{const rect=svg.getBoundingClientRect(),plotLeft=m.l/W*rect.width,plotRight=(W-m.r)/W*rect.width,px=Math.max(plotLeft,Math.min(plotRight,ev.clientX-rect.left)),vx=px/rect.width*W,target=minT+(vx-m.l)/(W-m.l-m.r)*(maxT-minT),nearest=rows.reduce((a,b)=>Math.abs(b.time_ms-target)<Math.abs(a.time_ms-target)?b:a,rows[0]),xx=x(nearest.time_ms);cross.setAttribute('x1',xx);cross.setAttribute('x2',xx);cross.setAttribute('visibility','visible');const items=allDefs.filter(d=>nearest[d[0]]!=null).map(d=>`<div class="gx-tooltip-row"><span>${d[1]}</span><strong>${fmt(nearest[d[0]],d[3])}${d[2]}</strong></div>`);if(cmp&&nearest[cmp]!=null)items.push(`<div class="gx-tooltip-row"><span>${COMPARES[compareKey]}</span><strong>${fmt(nearest[cmp],cmpMeta[3])}${cmpMeta[2]}</strong></div>`);tooltip.innerHTML=`<b>${currentData.kind==='daily'?dateFmt(nearest.time_ms,true):dateTimeFmt(nearest.time_ms)}</b>${items.join('')}`;tooltip.style.display='block';const crossPx=xx/W*rect.width,tw=tooltip.offsetWidth||185;tooltip.style.left=(crossPx+tw+24<rect.width?crossPx+12:crossPx-tw-12)+'px';tooltip.style.top='12px'})
...
function render(){
 const cat=CATS[category],pLabel=period==='custom'?`${$('#gx-from').value||'…'} → ${$('#gx-to').value||'…'}`:(manifest?.period_labels?.[period]||period);
...
function initUI(){
 ...
 document.querySelectorAll('#gx-periods button').forEach(b=>b.onclick=()=>{const p=b.dataset.period;activeKeys=new Set();compareKey='';$('#gx-custom-range').hidden=p!=='custom';if(category==='agro'&&['24h','7d','30d'].includes(p))category='temp';if(p!=='custom')loadPeriod(p)});$('#gx-apply-range').onclick=()=>{activeKeys=new Set();compareKey='';loadPeriod('custom')};
...
async function init(){
 ...
 const now=new Date(),ago=new Date(now.getTime()-30*86400000),iso=d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;$('#gx-from').value=iso(ago);$('#gx-to').value=iso(now);await loadPeriod('24h');
}
init();
