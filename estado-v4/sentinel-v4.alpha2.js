...
function render(){
 ...
 const ec=report.external_temperature_context;$('#s-temp-context').textContent=ec?`IPMA regional ${fmt(ec.regional_temperature_min_c)}–${fmt(ec.regional_temperature_max_c)} °C · ${fmt(ec.distance_km_min,0)}–${fmt(ec.distance_km_max,0)} km`:'sem contexto regional suficiente';
 $('#s-ipma-count').textContent=report.source_context?.regional_station_count??report.source_context?.nearby_station_count??0;
 ...
 $('#s-ipma').innerHTML=(report.nearby_ipma||[]).map(x=>`<div class="station"><div><strong>${x.station_name}</strong><span>${fmt(x.distance_km)} km · ${x.elevation_m==null?'altitude IPMA não disponível':fmt(x.elevation_m,0)+' m'} · ${x.observed_utc||'hora indisponível'}</span></div><div class="right"><strong>${fmt(x.temperature)} °C</strong><span>HR ${fmt(x.humidity,0)}% · vento ${fmt(x.wind_kmh)} km/h</span></div></div>`).join('')||'<div class="station"><span>Sem observações IPMA disponíveis.</span></div>';
 const gpu=caps.gpu||{},dbc=caps.database||{},ml=caps.ml||{},cam=report.camera_context||{},cc=cam.classification||{};
 $('#s-cap').innerHTML=[
  ['SQLite',dbc.persistent_mount_active?'Persistência ativa':'Persistência por configurar',dbc.warning||dbc.path],
  ['Camera AI',cc.available?`ativa em ${cc.device||'—'}`:'serviço Vision separado / ainda sem classificação',gpu.note],
